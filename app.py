...
def load_user_progress(username):
    users = load_users()
    if username in users:
        progress = users[username].get("progress", {})
...
def show_home():
    ...
    # Number of questions
    max_q = min(50, len(questions))  # Reduced max for better performance
    num_q = st.slider("Number of questions", 5, max_q, 10, 5)
...
def show_quiz():
    ...
    with col1:
        if st.button("🏠 Home"):
            if save_user_progress(st.session_state.username):
    ...
    st.markdown(f"**{system}** » **{subject}** » {topic}")
    if "difficulty" in q:
    ...
    for i, opt in enumerate(options):
        ...
        if st.button(
            label,
            key=f"opt_{i}_{quiz_state['idx']}",  # Added idx to key for uniqueness
    ...
    # Quick jump (only if more than 1 question)
    if len(quiz_questions) > 1:
        ...
        selected_q = st.selectbox(
            ...
            key=f"jump_select_{quiz_state['idx']}"
        )
...
def show_results(quiz_questions):
    ...
    if marked:
        ...
        for q_id in marked:
            ...
            if question:
                idx = quiz_questions.index(question) + 1
                st.write(f"**Q{idx}:** {question['question'][:100]}...")
...
def show_performance_analysis():
    ...
    if progress["performance_by_system"]:
        ...
        st.info("No system performance data yet. Complete some quizzes!")
    ...
    if progress["performance_by_subject"]:
        ...
        st.info("No subject performance data yet. Complete some quizzes!")
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.logged_in:
            ...
            st.divider()
            st.caption(f"Questions loaded: {len(questions)}")
        else:
            st.info("Please login to save your progress")
    ...
    elif st.session_state.quiz_config["quiz_started"]:
        show_quiz()
    else:
        show_home()
